  ...
  return (
    <section id="about" className="border-b border-ink">
      <div className="grid lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
        <header className="border-b border-ink px-4 py-8 sm:px-6 sm:py-10 lg:border-r lg:border-b-0 lg:px-10">
          <p className="font-mono text-section font-medium uppercase tracking-widest text-muted">
            02 — About
          </p>
          <h2 className="mt-3 font-display text-display tracking-display">The loop, closed</h2>
        </header>
        <div className="px-4 py-8 sm:px-6 sm:py-10 lg:px-10">
          <p className="max-w-xl text-base leading-relaxed sm:text-lg">{profile.summary}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 border-t border-ink sm:grid-cols-4">
        {stats.map((stat, index) => (
          ...
          >
            <p className="font-display text-4xl tracking-tight tabular-nums sm:text-5xl">{stat.value}</p>
            <p className="mt-2 font-mono text-section uppercase tracking-widest text-muted">
              {stat.label}
            </p>
          </div>
        ))}
      ...
      <div className="border-t border-ink">
        <div className="border-b border-ink px-4 py-3 sm:px-6 lg:px-10">
        ...
        <ul>
          {experience.map((item) => (
            ...
            >
              <p className="font-mono text-section uppercase tracking-widest text-muted">{item.period}</p>
              <div>
